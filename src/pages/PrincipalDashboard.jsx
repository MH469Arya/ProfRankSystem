...
export default function PrincipalDashboard() {
    ...
    const renderContent = () => {
        switch (activeTab) {
            ...
            default: return <DepartmentManager />;
        }
    };
    ...
    const TabButton = ({ id, label }) => (
        <button
            ...
            className={`px-6 py-3 text-sm font-bold uppercase tracking-wider border-b-2 transition-colors ${activeTab === id ? 'border-black text-black' : 'border-transparent text-gray-400 hover:text-gray-600'
                }`}
        >
            {label}
        </button>
    ...
    return (
        <div className="min-h-screen bg-white flex flex-col">
            <header className="bg-black text-white p-4 flex justify-between items-center sticky top-0 z-10 shadow-none">
                <h1 className="text-xl font-bold uppercase tracking-wide">Principal Dashboard</h1>
                <button
                    onClick={logout}
